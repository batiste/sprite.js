...
function rectangleEdges(rect) {
    // Return the 4 edges coordinate of the rectangle accroding the the rotation
...
function rectangleCollides(a, b) {
    ...
    if(a.angle == 0 && b.angle == 0) {
        if(a.x > b.x) {
            var x_inter = a.x - b.x < a.w;
        } else {
            var x_inter = b.x - a.x < b.w;
        }
        if(x_inter == false)
            return false;
...
Sprite.prototype.isPointIn = function pi(x, y) {
